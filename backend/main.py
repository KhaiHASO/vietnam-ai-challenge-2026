...
from ai_layer.tools.db_mock import load_db, save_db, get_storage_status
...
def startup_event():
    ...
    if settings.SKIP_RAG_SEED:
        print("[Backend] Skipping RAG seed during startup.")
    else:
        seed_knowledge_base()
    load_db()
...
def health_check():
    """
    Check FastAPI, active domain, and database connectivity.
    """
    return {
        "success": True,
